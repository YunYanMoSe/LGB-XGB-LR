...
`      "category": "家居装饰",`
`      "code": "21332",`
`      "desc": "MOROCCAN BEATEN METAL MIRROR",`
`      "desc_zh": "摩洛哥锤打金属镜子",`
`      "price": 12.75,`
`      "rank": 1,`
`      "score": 0.4951`
...
`      "category": "",`
`      "code": "21692",`
`      "desc": "SILVER LATTICE VANILLA CANDLE POT",`
`      "desc_zh": "SILVER LATTICE VANILLA CANDLE POT",`
`      "price": 3.75,`
`      "rank": 2,`
`      "score": 0.484`
`    },`
...
`{`
`  "error": "缺少 item_id 参数，例如 /similar?item_id=85123A&topn=5"`
`}`
...
- 商品不在嵌入词表（出现次数 < 5 的冷门品无向量，仅 `method=item2vec`）→ 404：
  `{`
`  "error": "商品 ZZZZ9 不在嵌入词表（出现次数 < 5 的冷门品没有向量）"`
`}`
...
`      "category": "手工用品",`
`      "code": "23569",`
`      "desc": "TRADTIONAL ALPHABET STAMP SET",`
`      "desc_zh": "传统字母印章套装",`
`      "price": 4.95,`
`      "proba": 0.8474,`
`      "rank": 1,`
...
`      "category": "装饰品",`
`      "code": "84879",`
`      "desc": "ASSORTED COLOUR BIRD ORNAMENT",`
`      "desc_zh": "彩色小鸟装饰品",`
`      "price": 1.69,`
`      "proba": 0.8406,`
`      "rank": 2,`
...
`      "category": "派对用品",`
`      "code": "22086",`
`      "desc": "PAPER CHAIN KIT 50'S CHRISTMAS",`
`      "desc_zh": "纸链套装50年代圣诞",`
`      "price": 2.95,`
`      "proba": 0.8239,`
`      "rank": 3,`
...
`  "topn": 3,`
`  "user": {`
`    "last_basket_zh": "兔子夜灯、小号玻璃钟罩、粉色悬挂心形茶烛台、波点彩旗、经典金属鸟笼植物架",`
`    "n_invoices": 4,`
`    "n_items": 184`
`  },`
`  "user_id": 12597`
...
`          "category": "厨房用品",`
`          "code": "22720",`
`          "desc": "SET OF 3 CAKE TINS PANTRY DESIGN",`
`          "desc_zh": "3件装蛋糕罐食品储藏室设计",`
`          "hit": false,`
`          "price": 4.95,`
`          "rank": 3`
`        }`
...
`          "category": "厨房用品",`
`          "code": "23245",`
`          "desc": "SET OF 3 REGENCY CAKE TINS",`
`          "desc_zh": "3个摄政时期蛋糕罐套装",`
`          "hit": false,`
`          "price": 4.95,`
`          "rank": 1`
...
`          "category": "生活用品",`
`          "code": "22619",`
`          "desc": "SET OF 6 SOLDIER SKITTLES",`
`          "desc_zh": "套装6个士兵造型保龄球",`
`          "hit": false,`
`          "price": 3.75,`
`          "rank": 1`
...
`          "category": "",`
`          "code": "84522",`
`          "desc": "PINK PARTY SUNGLASSES",`
`          "desc_zh": "PINK PARTY SUNGLASSES",`
`          "hit": false,`
`          "price": 1.95,`
`          "rank": 2`
`        },`
...
`  ],`
`  "user": {`
`    "last_basket_zh": "传统木质跳绳、6个木制保龄球棉布袋装、保持冷静热水袋、彩色小鸟装饰品",`
`    "n_invoices": 2,`
`    "n_items": 19`
`  },`
`  "user_id": 13899`
...
### 5.1 健康检查（T1）
![健康检查 /health（HTTP 200）](api_self_test/shots/T1_health.png)
> 请求 GET /health → HTTP 200，status=ok、ready=true
### 5.2 商品相似（T2 item2vec / T3 itemcf）
![商品相似 Item2Vec（HTTP 200）](api_self_test/shots/T2_similar_i2v.png)
> 请求 /similar?item_id=85123A&topn=3&method=item2vec&format=json → HTTP 200，返回 3 条相似商品与相似度
![商品相似 ItemCF（HTTP 200）](api_self_test/shots/T3_similar_itemcf.png)
> 请求 /similar?item_id=85123A&topn=3&method=itemcf&format=json → HTTP 200
### 5.3 精排推荐（T4 LR / T5 LightGBM）
![精排推荐 LR（HTTP 200）](api_self_test/shots/T4_recommend_lr.png)
> 请求 /recommend?user_id=12597&topn=3&ranker=lr&format=json → HTTP 200，Top-3 含 proba 与命中路由
![精排推荐 LightGBM（HTTP 200）](api_self_test/shots/T5_recommend_lgb.png)
> 请求 /recommend?user_id=12597&topn=3&ranker=lgb&format=json → HTTP 200，结构同 LR
### 5.4 基线对照（T6）
![基线对照（HTTP 200）](api_self_test/shots/T6_baseline.png)
> 请求 /baseline?user_id=13899&topn=3&format=json → HTTP 200，三条基线并列
### 5.5 错误处理（反例 E1–E4）
![错误用例 E1（HTTP 404）](api_self_test/shots/E1_similar_no_item.png)
> 反例：缺 item_id → HTTP 404，error=缺少 item_id 参数
![错误用例 E2（HTTP 404）](api_self_test/shots/E2_similar_cold.png)
> 反例：冷门品 ZZZZ9 无向量 → HTTP 404，error=不在嵌入词表
![错误用例 E3（HTTP 404）](api_self_test/shots/E3_recommend_nouser.png)
> 反例：用户 99999999 不存在 → HTTP 404
![错误用例 E4（HTTP 404）](api_self_test/shots/E4_recommend_ranker.png)
> 反例：ranker=svm 非法 → HTTP 404

"""候选集打分 v5 提交：LGB(macro,全月)×0.15 + XGB(macro,全月)×0.15 + LR(macro_sp,最近2月)×0.7。

依据（scripts/44 三分量 OOF）：
    * xgb_macro 单模型 gAUC_m .8586 > lgb_macro .8556，为去相关且更强的树分量；
    * (lgb .15, xgb .15, lr .7) mgAUC=.8710 > v4 两分量 .8703（gAUC_w .8571 亦最高）；
    * LR 仍为主导（近窗 .8684），树份额在 LGB/XGB 间对半去相关。
打分 cut=11-01（leader_clean 全量），特征 44，58,205 全行覆盖。

用法：venv\\Scripts\\python.exe scripts\\45_build_submission_v5.py
产物：outputs/candidate/sample_submission_cand-align-v5-fuse.csv + models/candidate_*.pkl + meta
"""
...
NAME = "cand-align-v5-fuse"
W = {"lgb_macro": 0.15, "xgb_macro": 0.15, "lr": 0.7}   # scripts/44 mgAUC 0.8710
...
def main() -> None:
    ...
    print(f"[1/4] LGB(macro, 全月) 完成 {len(tr_all):,} 行", flush=True)
    ...
    clf_xgb.fit(tr_all[FEATS], tr_all["label"],
                sample_weight=macro_weight(tr_all, with_sp=False))
    print(f"[2/4] XGB(macro, 全月) 完成", flush=True)
    ...
    print(f"[3/4] LR(macro_sp, 最近{LR_LAST_N}月={MONTHS[-LR_LAST_N:][0][:7]}.."
    ...
    N = len(df)
    fused = (W["lgb_macro"] * rankdata(s_lgb) / N
             + W["xgb_macro"] * rankdata(s_xgb) / N
             + W["lr"] * rankdata(s_lr) / N)
    ...
    meta = {"name": NAME,
            "desc": "fusion LGB(macro,all) .15 + XGB(macro,all) .15 + LR(macro_sp,last2) .7 "
